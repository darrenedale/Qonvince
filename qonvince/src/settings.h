...
namespace Qonvince {
	...
	:	public QObject {
		...
		public:
			...
			Q_PROPERTY(bool quitOnMainWindowClosed READ quitOnMainWindowClosed WRITE setQuitOnMainWindowClosed)
			Q_PROPERTY(bool startMinimised READ startMinimised WRITE setStartMinimised)
			Q_PROPERTY(bool copyCodeOnClick READ copyCodeOnClick WRITE setCopyCodeOnClick)
			Q_PROPERTY(bool hideOnCodeCopyClick READ hideOnCodeCopyClick WRITE setHideOnCodeCopyClick)
			Q_PROPERTY(bool clearClipboardAfterInterval READ clearClipboardAfterInterval WRITE setClearClipboardAfterInterval)
			Q_PROPERTY(int clipboardClearInterval READ clipboardClearInterval WRITE setClipboardClearInterval)
			Q_PROPERTY(CodeLabelDisplayStyle codeLabelDisplayStyle READ codeLabelDisplayStyle WRITE setCodeLabelDisplayStyle)
			Q_PROPERTY(int codeRevealTimeout READ codeRevealTimeout WRITE setCodeRevealTimeout NOTIFY codeRevealTimeoutChanged)

			inline bool singleInstance( void ) const {
				return m_singleInstance;
			}

			inline bool quitOnMainWindowClosed( void ) const {
				return m_quitOnMainWindowClosed;
			}

			inline bool startMinimised( void ) const {
				 return m_startMinimised;
			}

			inline bool copyCodeOnClick( void ) const {
				return m_copyCodeOnClick;
			}

			/* if codes are copied on click, does the user also want
			 * to hide the app when this happens? */
			inline bool hideOnCodeCopyClick( void ) const {
				return m_hideOnCodeCopyClick;
			}

			/* when a code is copied to the clipboard, clear the clipboard
			 * after a specified interval */
			inline bool clearClipboardAfterInterval( void ) const {
				return m_clearClipboardAfterInterval;
			}

			/* how many seconds to wait after a copy before clearing the
			 * clipboard */
			inline int clipboardClearInterval( void ) const {
				return m_clipboardClearInterval;
			}

			inline CodeLabelDisplayStyle codeLabelDisplayStyle( void ) const {
				return m_codeLabelDisplayStyle;
			}

			inline int codeRevealTimeout( void ) const {
